...
### Bridge
機能を提供するクラスと、実装を提供するクラスを独立させるためのパターン<br>
目的と手段を分離。委譲を使うことで機能と実装を橋渡ししているように見える<br>
構造に関するデザインパターン


- オブジェクト指向的要素
  - 継承、ポリモーフィズムと委譲を利用したパターン
- メリット
  - 機能と実装が分離されているので修正が容易
  - 実行時に実装を切り替えられる
  - 機能や実装のバリエーションが豊富な場合、最終的に作成すべきクラス数を抑えることができる
- デメリット
  - 機能や実装にバリエーションが少ない場合、余計なクラス数が増えて複雑になる
- 使い所
  - 機能と実装の組み合わせが多い場合
...
### FlyWeight
ボクシングのフライ級を意味する<br>
インスタンス化されたオブジェクトを効率よく共有することで、リソースの消費を抑える。<br>
メモリの使用量を軽くする。環境は状況によって変化しない情報のみ共有する<br>
構造に関するデザインパターン
...
- デメリット
  - コードが複雑化
  - 共有されているオブジェクトの変更によって想定外のバグを生む(変更しない方が良い)
- 使い所
  - 同一のオブジェクトお大量に使用する必要がある場合
  - インスタンス生成によるリソースを抑えたい場合
    - パフォーマンスチューニングも


### Observer
観察対象のオブジェクトの状態変化が発生した際に、複数の観察者に対して通知を行う<br>
観察よりも通知に重点が置かれているPub/Subパターンとも呼ばれる<br>
振る舞いに関するデザインパターン
...
- メリット
  ...
  - 新たな観測対象の追加や削除や修正が容易
- デメリット
  - 通知の順番に依存しない実装にしなければバグを生む可能性がある
    - 通知の順番をあまり意識しない実装を心がけること
- 使い所
  - 状態変化に応じた処理を行う
    - ニュースサイトやSNS
    - Angularに採用されテイルRxjs
...
### Mediator
関連し合うオブジェクト間のやりとりを仲介者となるオブジェクトに集約し、オブジェクトが直接やりとりすることを制限する<br>
制限することでオブジェクト同士の結びつきを弱くできる。 大家、住民、仲介業者的なイメージ<br>
振る舞いに関するデザインパターン
...
- デメリット
  - Mediatorが全てのオブジェクトに結合されたゴットクラスになる可能性を秘めている
- 使い所
  - 多くのオブジェクト同士が直接繋がっている場合
    - つながりをMediatorに集約、理解しやすく変更がしやすく、再利用性が高いものになる


### Memento
オブジェクトの現在の状態を保存しておき、状態が変化した場合でもその時の状態にも戻すことができる<br>
command+Zのようなひとつ戻る機能を実現。状態を生成するクラスとその履歴を管理するクラスを分離<br>
振る舞いに関するデザインパターン


- オブジェクト指向的要素
  - カプセル化の破壊を防ぐ(内部状態を公開することをいう)
    - 内部状態を非公開としたままスタップショットを作成できる
- メリット
  - カプセル化を破壊することなく、オブジェクトの状態のスナップショットを作成、復元できる
  - スナップショットの管理用クラスがあることでクラスをシンプルに保つことができる
- デメリット
  - Mementoを大量に作成してしまうとメモリ使用量が増加する
- 使い所
  - 復元する際にスナップショットを作成した場合
    - ゲームのセーブ
    - noteなど

### Visitor
データ構造を表すクラスとそれに対する操作を行うクラスを分離する<br>
操作を行うクラスがデータ構造を表すクラスを訪問して順に処理を行うことからvisitor<br>
データ構造を表すクラスには操作を記述する代わりに、訪問者を受け入れるためのメソッドを用意<br>
振る舞いに関するデザインパターン


- オブジェクト指向的要素
  - ダブルディスパッチを利用(訪問販売, 販売員がどの家に入るかによって別の処理をする)
    - 訪問者と受け入れる側がお互いのメソッドを呼び出す
- メリット
  - データ構造と操作を分離、共通な操作を一箇所にまとめることができる
  - 新しい操作を容易に追加可能
- デメリット
  - 新しいデータ構造を追加することは困難
- 使い所
  - ツリー構造のような複雑なオブジェクト構造の全ての要素に対して、ある操作を実行する必要がある場合
  - 特定のデータ構造に対して操作をいろいろ変化させたい場合
    - データ構造を固定して操作を変化させる


### Command
命令をメソッドではなく、独立したクラスとして表現する<br>
命令を送る側と受け取る側で分離。命令クラスのインスタンスを切り替えることで様々な処理を実現する<br>
ex: TVのリモコンなど<br>
振る舞いに関するデザインパターン
...
- メリット
  ...
  - 処理のキューイングが可能
- デメリット
  - 命令がシンプルだと複雑すぎる設計になりかねない
- 使い所
  - 複数の命令をひとまとめにして実行したい場合
  - 命令をキューに入れて遅延実行したい

### Interpreter
構文の解析を行って、その結果を利用して処理するパターン<br>
解析を行うための規則をクラスとして表現<br>
振る舞いに関するデザインパターン
