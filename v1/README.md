...
上位のモジュールは下位のモジュールに依存してはならない。どちらのモジュールも抽象に依存すべき。<br>
抽象は実装の詳細に依存してはならない。実装の詳細が抽象に依存すべきである

- 詳細に
  - 上位のモジュール
    - 人間に近い処理を担当(ビジネスルールやアプリケーションのユースケースなど)
    - アプリケーションの目的(なぜ存在するのか)を体現する
  - 下位のモジュール
    - コンピュータに近い処理を担当(データベースアクセス。アルゴリズム、外部通信など)
    - アプリケーションの目的達成の手段(どう実現するのか)を体現する
  - 例(相対的なので、絶対これが上位モジュールみたいなことはない)
    - 注文処理自体を上位モジュール
    - その中の、ユーザーの有効性チェック、在庫チェック、支払い処理、在庫更新処理などは下位モジュール
- 上位モジュールが下位モジュールに依存すること
  - 下位モジュールの変更が上位モジュールに影響を受ける構造になっている。
  - だが、よく考えるとおかしい話と言える
    - 手段の変更によって目的を変えなければいけないという構造になるから。決済方法によって注文できないものが存在するなんてことはないよね。
  - 依存の方向性を逆転させて、常に手段(下位モジュール)が目的(上位モジュール)に依存するべき
    - これを達成するための原則がDIPになる
- 抽象への依存とは
  - Pythonにおいては抽象クラスへの依存を意味する
  - インターフェースがある言語ではインターフェースに依存することをいう。
  - 抽象に依存するメリットデメリット
    - メリット
      - 下位の変更が上位に及ばない
      - 具象クラスよりも抽象クラスの方が変更されづらい
      - 具象クラスを切り替えられるようになる(OCP, LSP)
      - => よって変更に強くなる
      - 具象クラスに依存しないので、共同開発やテストがしやすい
    - デメリット
      - 抽象クラスを定義する必要があるので、コード量が増えるが
      - 他の原則と同様、正しく適用すれば基本的にメリットが上回る
- DIPと関係が深いデザインパターン(DI Dependency Injection)
  - どの具象クラスに依存するかは定義時には決まっていない。関数呼び出し時に具象クラスへの依存を注入する
  - モジュールの定義時には、抽象に依存しておいて(DIP)実行時に具象への依存をモジュールに注入する(DI)ことで、依存関係を動的に決定することができる
    - OCPが実現できる(ただし、LSPは前提)
  - notify関数として引数に抽象クラスを渡す。的な
  - どこから依存を注入するのか
    - 下記2つ、コンストラクタで注入されることが一番多い
    - コンストラクタ
    - メソッド
  - DIPの主な適用場所
    - データベースアクセスや、外部サービスの利用などの、技術的な詳細を扱う場合には、基本的にDIPを適用する
    - データベースや外部サービスへの依存は、テストケースを作りづらくするため、DIPを適用してテスト用の具象クラスと切り替えられた方が便利
    - Repositoryパターンが有名
...
### 最後に
- 上達する3つ
  - 原則を破るときは、最もな理由を用意してから破る
    - どうしても破る必要がある場合のみ、破っても良い。ただ安直に破ることは良くないよね。
